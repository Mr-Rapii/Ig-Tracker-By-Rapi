...
export default function NotFollowingBackArea() {
  const [status, setStatus] = useState<Status>("idle");
  const [progress, setProgress] = useState(0);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<PendingRequest[] | null>(null);
  ...
  const processFile = useCallback((file: File) => {
    ...
    xhr.open("POST", "/api/not-following-back");
    ...
    xhr.onload = () => {
      try {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          setResult(data.notFollowingBack);
  ...
  return (
    <div id="upload" className="mx-auto max-w-2xl scroll-mt-24">
          ...
          {status === "processing" ? (
            <div>
              <p className="font-display text-xl italic text-ivory">
                Membandingkan followers & following…
              </p>
              ...
              <p className="mt-3 text-xs text-ivory/40">Diproses di server, tidak disimpan setelah selesai.</p>
            </div>
          ) : (
            <div>
              <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-white/5">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="text-amber">
                  ...
                  <path d="M5 16v2a2 2 0 002 2h10a2 2 0 002-2v-2" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <p className="font-display text-xl italic text-ivory">Pilih file ZIP-mu</p>
              <p className="mt-2 text-sm text-ivory/50">Seret file ke sini, atau klik untuk memilih dari perangkatmu</p>
              <p className="mt-4 text-xs text-ivory/30">Maksimal 50MB · format .zip dari unduhan data Instagram</p>
            </div>
          )}
        </div>
      )}
      ...
      {status === "error" && error && (
        <p className="mt-4 rounded-lg border border-coral/30 bg-coral/10 px-4 py-3 text-sm text-coral">{error}</p>
      )}

      {status === "done" && result && (
        <div>
          {counts && (
            <p className="mb-4 text-center text-xs text-ivory/40">
              Kamu follow <span className="text-ivory/70">{counts.following}</span> akun,
              di-follow balik oleh <span className="text-ivory/70">{counts.followers}</span> akun
            </p>
          )}
          <ResultsList
            pending={result}
            onReset={reset}
            emptyTitle="Semua orang follow balik kamu."
            emptySubtitle="Tidak ada akun yang kamu follow tapi tidak follow balik."
            countSuffix="akun tidak follow balik"
          />
        </div>
      )}
    </div>
